...
const RoommateListing = ({ roommateItems, isDouble = true, onItemClick }) => {
  ...
  const splitItemsIntoRows = () => {
    if (isDouble) {
      ...
      if (window.innerWidth >= 1536) {
        itemsPerRow = Math.ceil(roommateItems.length / 3); // 3 rows for 2xl and above
      } else {
        itemsPerRow = Math.ceil(roommateItems.length / 2);
      }

      const cardHeight = 280; // Assume a fixed height for RoommateCard
      const maxRows = Math.floor((screenHeight - 80) / cardHeight);

      const adjustedItemsPerRow = Math.ceil(roommateItems.length / maxRows);

      const rows = [];
      for (let i = 0; i < roommateItems.length; i += adjustedItemsPerRow) {
        rows.push(roommateItems.slice(i, i + adjustedItemsPerRow));
      }
      return rows;
    } else {
      return [roommateItems]; // Single row
    }
  };
