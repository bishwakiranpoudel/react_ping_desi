  ...
  const splitItemsIntoRows = () => {
    if (isDouble) {
      ...
      if (window.innerWidth >= 1536) {
        itemsPerRow = Math.ceil(electronicsItems.length / 3); // 3 rows for 2xl and above
      } else {
        itemsPerRow = Math.ceil(electronicsItems.length / 2);
      }

      const cardHeight = 280; // Assume a fixed height for ElectronicCard
      const maxRows = Math.floor((screenHeight - 80) / cardHeight);

      const adjustedItemsPerRow = Math.ceil(electronicsItems.length / maxRows);

      const rows = [];
      for (let i = 0; i < electronicsItems.length; i += adjustedItemsPerRow) {
        rows.push(electronicsItems.slice(i, i + adjustedItemsPerRow));
      }
      return rows;
    } else {
      return [electronicsItems]; // Single row
    }
  };
